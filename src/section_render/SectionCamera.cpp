...
void SectionCamera::setupIsometric(const SectionPlane& plane,
                                    const AABB3& bbox,
                                    double scale_factor,
                                    int32_t img_width,
                                    int32_t img_height)
{
    ...
    // Build isometric view direction:
    //   Start from the plane normal, then rotate ~45° azimuth and ~35° elevation
    //   to show 3D depth while keeping the cut face visible.
    //
    // eye_dir = normalize(normal + u * tan(45°) + v * tan(35°))
    //         = normalize(normal + u * 1.0 + v * 0.7)

    Vec3 normal = plane.normal();
    Vec3 basis_u, basis_v;
    plane.getBasis(basis_u, basis_v);

    // View direction: look toward the KEPT side (negative normal)
    // with isometric rotation: azimuth ~45°, elevation ~35°
    // tan(45°) = 1.0, tan(35°) ≈ 0.7002
    view_dir_ = (-normal + basis_u * 1.0 + basis_v * 0.7002).normalized();

    // Camera coordinate frame:
    //   right  = axis_u_ (perpendicular to view_dir_ and up)
    //   up     = axis_v_ (perpendicular to view_dir_ and right)
    //
    // We want "up" to be roughly along basis_v.
    // right = normalize(view_dir × basis_v)
    // up    = normalize(right × view_dir)

    axis_u_ = view_dir_.cross(basis_v).normalized();
    axis_v_ = axis_u_.cross(view_dir_).normalized();

    // Project all 8 bbox corners onto camera axes to find view extent
    Vec3 corners[8] = {
        {bbox.min_pt.x, bbox.min_pt.y, bbox.min_pt.z},
        {bbox.max_pt.x, bbox.min_pt.y, bbox.min_pt.z},
        {bbox.min_pt.x, bbox.max_pt.y, bbox.min_pt.z},
...
Vec2 SectionCamera::project3D(const Vec3& p, double& depth) const
{
    double dx = p.x - origin_.x;
    double dy = p.y - origin_.y;
    double dz = p.z - origin_.z;
    ...
    // Depth = dot(diff, view_dir).  view_dir points from camera toward scene,
    // so closer points (shifted toward camera = against view_dir) give smaller
    // (more negative) depth → wins in Z-buffer test (depth < existing).
    depth = dx*view_dir_.x + dy*view_dir_.y + dz*view_dir_.z;
