...
def _parse_outputs(
    ...
    for eq in raw.get("element_quality", []):
        eq_list.append(ElementQualityData(
            ...
            aspect_measured=eq.get("aspect_measured", False),
            peak_aspect_ratio=eq.get("peak_aspect_ratio", 0.0),
            aspect_unavailable_count=eq.get("aspect_unavailable_count", 0),
            jacobian_measured=eq.get("jacobian_measured", False),
            min_jacobian=eq.get("min_jacobian", 1.0),
            jacobian_unavailable_count=eq.get("jacobian_unavailable_count", 0),
            warpage_measured=eq.get("warpage_measured", False),
            peak_warpage=eq.get("peak_warpage", 0.0),
            skewness_measured=eq.get("skewness_measured", False),
            peak_skewness=eq.get("peak_skewness", 0.0),
            volume_measured=eq.get("volume_measured", False),
