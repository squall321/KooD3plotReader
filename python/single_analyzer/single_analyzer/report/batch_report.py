"""Batch summary HTML report for multiple single_analyzer runs."""
...
def generate_batch_html(
    results: list[dict],       # list of result.json dicts (single_analyzer/1.0)
    ...
    for p in sorted(output_root.iterdir()):
        ...
        if p.is_dir() and rj.exists():
            try:
                ...
                if d.get("schema", "").startswith("single_analyzer"):
                    results.append(d)
...
<header>
  <div>
    ...
    <div class="sub">single_analyzer — {output_root}</div>
  </div>
</header>
<div class="container">
