...
def main() -> None:
    ...
    if not argv or command == "gui":
        try:
            ...
            print("사용법: single_analyzer <path> [options]", file=sys.stderr)
            print("도움말: single_analyzer --help", file=sys.stderr)
            sys.exit(1)
        return
...
def _parse_config_yaml(path: str) -> dict:
    """Parse single_analyzer YAML config file into a flat dict.
...
def _apply_config_to_args(args: argparse.Namespace) -> None:
    ...
    print(f"[single_analyzer] config: {p}")
...
def _resolve_install_dir(explicit: str) -> Path | None:
    ...
    use its parent as install root (e.g. installed/bin/single_analyzer → installed/).
    """
    if explicit:
        p = Path(explicit).resolve()
        if p.is_dir():
            return p
    # Auto-detect: script in .../bin/single_analyzer → parent is .../bin/ → grandparent is install root
...
def run_single(args: argparse.Namespace) -> None:
    ...
    if not args.path:
        ...
        print("       또는: single_analyzer --config analysis.yaml", file=sys.stderr)
        sys.exit(1)
...
def _aggregate(
    ...
    if sim_info.d3plot:
        ...
        if kw_data:
            ...
            print(f"[single_analyzer] keyword: {n_part} parts, {n_mat} materials parsed from {kw_data.source_path}")
