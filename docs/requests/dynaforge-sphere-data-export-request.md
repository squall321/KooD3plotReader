...
## 9. 있을 때 / 없을 때 — UI 가 데이터 유무에 맞게 동작 (2026-09-04)

시계열 5종을 추가하고 나니 **화면 쪽 처리**가 남았습니다. 없는 지표를 선택지에 남겨 두면
고르는 순간 빈 차트가 나와 "값이 0" 으로 오독됩니다.

### 9-1. Mollweide 지표 — 이미 되어 있었음

`hasQty()` 가 데이터셋을 훑어 값이 하나도 없는 지표를 선택지에서 감춥니다.
σ1·σ3·ε1·ε3·ε_vm 5종이 이 규율을 이미 따르고 있었습니다.

### 9-2. 시계열 탭 — 이번에 맞춤

종전에는 `stress/strain/g/disp` **4종이 무조건** 떴습니다. 운동 데이터가 없는
데이터셋에서도 "G-Force" 가 선택지에 남아 빈 차트를 그렸습니다.

- `TH_QTYS` 표로 지표를 정의하고 `hasSeries()` 로 **실제 데이터가 있는 것만** 노출
- 신규 5종(σ1·σ3·ε1·ε3·ε_vm)을 목록에 추가 — 압축측(σ3·ε3)은 `min` 을 그림
- 선택 중인 지표가 그 데이터셋에 없으면 **가용한 첫 지표로 자동 전환**
- 시계열이 하나도 없으면 "분석 시 CSV 출력을 켜야 합니다" 안내

### 9-3. 실측 — 두 경우

같은 런으로 CSV 주입본(있을 때)과 원본(없을 때)을 만들어 브라우저로 확인했습니다.

| | payload 시계열 | 시계열 탭 선택지 | 신규 5종 렌더 |
|---|---|---|---|
| **있을 때** | `principal_ts`·`vm_strain_ts` 등 실림 | **9종** | 5/5 그려짐 |
| **없을 때** | 키 자체가 없음 | **4종** (stress/strain/g/disp) | 선택지에 없음 |

없을 때 `principal` 을 강제로 지정해도 `stress` 로 되돌아가 빈 차트가 나오지 않습니다.

즉 **데이터가 있으면 전부 쓰이고, 없으면 조용히 0 을 그리는 대신 아예 감춥니다.**
