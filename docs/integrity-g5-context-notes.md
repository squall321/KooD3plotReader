...
## F3 각도 이름 충돌 (2026-09-17)
- 이름은 **각도를 모를 때만** 믿는다. 각도가 둘 다 있으면 `_ANGLE_EXACT_DEG`(1e-3°)
  안에 들어야 실측 일치다. impact 처럼 각도가 없는 번들은 예전 그대로 동작한다.
- identity 판정에도 같은 검사를 건다. 이름 집합이 같아도 방향이 다르면 identity 가
  아니라 nearest/off/idw 로 내려간다.
- 강등은 조용히 하지 않는다 — `angle_name_collision` WARN 에 쌍 수·최대 각차·
  대표 이름을 적는다.

## F4 IDW 지표 누락 (2026-09-17)
- `_idw_sample` 이 `METRIC_KEYS` 전체를 돈다. 압축측도 부호 있는 값을 그대로
  가중평균하므로 음수가 유지된다(따로 절댓값을 쓰지 않는다).

## F5 sphere 사이드카 단위 (2026-09-17)
- 사이드카가 `unit_system` + `unit_labels` 를 싣는다. **acc 만은 "G"** 다 —
  peak_g 를 이미 G 로 나눠 저장하기 때문에 덱의 가속도 단위를 적으면 1e6 배
  과대 표기가 된다(이 규칙은 기존 시험이 지키고 있었다).
- 미검출이면 unit_labels 키를 만들지 않는다. federate 어댑터도 기본값을 채우지
  않고, compare 가 `unit_unlabeled` WARN 으로 "모른다" 를 말한다.
- 규칙이 뒤집히면서 기존 시험 2건(기본값을 채우는지 확인하던 것)을 새 규칙으로
  갱신했다. 대신 helpers 의 최소 sphere 사이드카에 unit_labels 를 넣어,
  '지금의 koo_sphere_report 가 내보내는 모양' 을 픽스처가 반영하게 했다.

## C1 방향도 축 (2026-09-17)
- 축 범위를 **데이터에서** 정한다(|값|>90 이면 ±180, 아니면 ±90). sphere 로더의
  자동 판정(`swap_axes`)과 같은 착상이되, 여기서는 축을 바꾸는 대신 범위를 넓힌다 —
  라벨(roll/pitch)이 그대로라 읽는 사람이 헷갈리지 않는다.
- 클램프는 하지 않는다. 범위를 데이터로 잡으면 밖으로 나갈 점이 없고, 억지로
  가장자리에 붙이면 없는 자세를 그리는 셈이 된다.
