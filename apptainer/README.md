...
```
apptainer/
├── SmartTwinPostprocessor_headless.def    # Slurm 노드용 (GUI 없음, ~345MB)
├── SmartTwinPostprocessor_full.def        # 데스크탑/viz 노드용 (GUI 포함, ~500MB)
├── build.sh                        # 빌드 래퍼
├── examples/
...
# → apptainer/SmartTwinPostprocessor_headless.sif (~345MB)

# viz 노드용 (GUI 포함)
./apptainer/build.sh full
# → apptainer/SmartTwinPostprocessor_full.sif (~500MB)
...
| 단계 | 경로 |
|------|------|
| **빌드 직후** (프로젝트 내) | `<project_root>/apptainer/SmartTwinPostprocessor_{headless,full}.sif` |
| **공유 배포 경로 (권장)** | `/opt/apptainers/SmartTwinPostprocessor_{headless,full}.sif` |
...
sudo cp apptainer/SmartTwinPostprocessor_headless.sif /opt/apptainers/
sudo cp apptainer/SmartTwinPostprocessor_full.sif /opt/apptainers/

# 또는 NFS/공유 저장소에
cp apptainer/SmartTwinPostprocessor_headless.sif /nfs/shared/containers/
```
...
```bash
# 방법 1: 환경변수 (스크립트에서)
SIF=/opt/apptainers/SmartTwinPostprocessor_headless.sif
apptainer exec --bind /data "$SIF" post_analyze /data/test_001

# 방법 2: 절대 경로 직접
apptainer exec --bind /data /opt/apptainers/SmartTwinPostprocessor_headless.sif post_analyze /data/test_001
...
apptainer exec --bind /data:/data \
    SmartTwinPostprocessor_headless.sif \
...
```bash
# 단일 d3plot 분석만
apptainer exec --bind /data:/data SmartTwinPostprocessor_headless.sif \
...
```bash
apptainer run SmartTwinPostprocessor_headless.sif
# VERSION 정보 + 사용법 출력
```
...
```bash
# 기본 (데이터만)
apptainer exec --bind /data:/data SmartTwinPostprocessor_headless.sif post_analyze /data/...

# 스크래치 디렉토리도 추가
apptainer exec --bind /data:/data,/scratch:/scratch \
    SmartTwinPostprocessor_headless.sif post_analyze /data/...

# GUI 필요 시 (full 이미지, viz 노드)
apptainer exec --nv --bind /tmp/.X11-unix,/data:/data \
    SmartTwinPostprocessor_full.sif koo_viewer sphere /data/drop_test_001
```
...
```bash
# 빌드한 이미지 → 공유 경로로 복사
cp apptainer/SmartTwinPostprocessor_headless.sif /opt/apptainers/
```
...
local sif = "/opt/apptainers/SmartTwinPostprocessor_headless.sif"
